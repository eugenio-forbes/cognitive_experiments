...
### Function to gather stimulation mode
def prompt_stimulation_type():
    valid_input = False
    while not valid_input:
        valid_input = True
        print("Choose stimulation type.")
        print("1: Single Frequency")
        print("2: Two Frequencies")
        print("3: Binary Noise")
        print("4: Two Frequencies and Binary Noise")
        print("5: Theta Burst")
        user_input = input("Enter number:")
        if user_input != '':
            if not user_input.isnumeric() or int(user_input) < 1 or int(user_input) > 5:
                valid_input = False
                print("Must enter a number between 1 and 5. Try again.")
            else:
                if user_input == '1':
                    stimulation_type = 'single_frequency'
                elif user_input == '2':
                    stimulation_type = 'two_frequencies'
                elif user_input == '3':
                    stimulation_type = 'binary_noise'
                elif user_input == '4':
                    stimulation_type = 'two_frequencies_and_binary_noise'
                elif user_input == '5':
                    stimulation_type = 'theta_burst'                 
        else:
            valid_input = False
            print("Must select stimulation type. Try again.")
    return stimulation_enabled
