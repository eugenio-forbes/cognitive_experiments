"""
Item Consolidation Experiment Subroutines
...
@Subroutine
def DisplayTextInstructions(self, instructions_text):
    ### General Instructions
    with Parallel():
        ScrollView(do_scroll_x=False)
        RstDocument(text=general_instructions_text, size=screen_size, base_font_size=instructions_font_size, do_scroll_y=True)
    with UntilDone():
...
### Subroutine to display environment image/noun pairs and gather response for noun type
@Subroutine
def ExperimentPhase(self, experiment_block, experiment_phase_keys):
    with Loop(experiment_block) as trial:
        ...
        environment_source = environment_directory + trial.current['environment'] + '.png'
        with Parallel():
            environment_image = Image(source=environment_source, size=environment_size, duration=allowed_response_time, top=top, blocking=False)
            Label(text=trial.current['noun'], duration=allowed_response_time, font_size=large_font, bottom=label_offset, blocking=False)
            ### Gather response to trial
            trial_response = KeyPress(keys=experiment_phase_keys, base_time=orient_image.appear_time['time'], blocking=False)
        ...
        with Parallel():
            ...
            Log(name='event',
                ...
                session=trial.current['session'],
                experiment_phase=trial.current['experiment_phase'],
                event=trial.current['event'],
                trial_index=trial.current['trial_index'],
                noun=trial.current['noun'],
                noun_type=trial.current['noun_type'],
                noun_category=trial.current['noun_category'],
                environment=trial.current['environment'],
                test_condition=trial.current['test_condition'],
                response=trial_response.pressed,
                time_to_respond=time_to_respond,
                trial_time=environment_image.appear_time['time'] * 1000)
