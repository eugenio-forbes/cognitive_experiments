...
def update_list_from_checkpoint(experiment_block_list, checkpoint_file, session_lock_file):
    ...
    if last_experiment_phase is not None:
        if last_experiment_phase == 'STUDY1':
            if last_trial_index + 1 < len(experiment_block_list['STUDY1']):
                trial_index = last_trial_index + 1
            else:
                experiment_phase = 'STUDY2'
        elif last_experiment_phase == 'STUDY2':
            if last_trial_index + 1 < len(experiment_block_list['STUDY2']):
                experiment_phase = 'STUDY2'
                trial_index = last_trial_index + 1
            else:
                experiment_phase = 'TEST'
        if last_experiment_phase == 'TEST':
            if last_trial_index + 1 < len(experiment_block_list['TEST']):
                experiment_phase = 'TEST'
                trial_index = last_trial_index + 1
            else:
    ...
    experiment_block = experiment_block_list[experiment_phase][trial_index:]

    if experiment_phase == 'TEST':
        experiment_phase_keys = [new_key, same_key, rearranged_key]
    else:
        experiment_phase_keys = [animate_key, inanimate_key]

    return experiment_block, experiment_phase, experiment_phase_keys
