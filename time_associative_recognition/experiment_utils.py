...
def prompt_n_experiment_blocks():
    ...
    while not valid_input:
        ...
        if n_experiment_blocks is not None:
            if not n_experiment_blocks.isnumeric() or int(n_experiment_blocks) < 0 or int(n_experiment_blocks) > 6:
                valid_input = False
                print("Must enter an integer between 1 and 6. Try again.")
        else:
            valid_input = False
            print("Must enter number of experiment blocks for this session. Try again.")
...
def update_list_from_checkpoint(experiment_block_list, checkpoint_file, session_lock_file):
    ...
    ### Use gathered checkpoint information to remove previous items from stimulus list
    if last_experiment_block is not None:     
        if last_experiment_phase == 'ENCODING':
            if last_trial_index + 1 < len(experiment_block_list[0]['study_phase']):
                study_phase = experiment_block_list[last_experiment_block]['study_phase']
                new_study_phase = study_phase[last_trial_index + 1:]
                ...
                experiment_block_list = experiment_block_list[last_experiment_block:]
        elif last_experiment_phase == 'RETRIEVAL':
            if last_trial_index + 1 < len(experiment_block_list[0]['test_phase']):
                skip_study_phase = True
                test_phase = experiment_block_list[last_experiment_block]['test_phase']
                new_test_phase = test_phase[last_trial_index + 1:]
                ...
                if last_experiment_block + 1 < len(experiment_block_list):
                    ...
                    print(f"Subject {subject} had already completed session {session}.")
