"""
Time Associative Recognition Experiment
...
session_keys_dictionary_file = session_directory + 'session_keys_dictionary.json'
...
if not os.path.isdir(session_directory):
    ...
    ### Gather a valid number of blocks (1-6) to be performed in this session:
...
### If the experimental session has been already completed, this will be indicated by existence of a lock file
...
### Retrieve stimulus list from saved file
with open(experiment_block_list_file, 'r') as file_handle:
    experiment_block_list = json.load(file_handle)

### Retrieve keys and labels assigned to session
with open(session_keys_dictionary_file, 'r') as file_handle:
    session_keys_dictionary = json.load(file_handle)
...
### Initialize variables used by experiment during execution            
experiment.lucky_number = 0
experiment.delay_duration = 0
...
with Loop(experiment_block_list) as experiment_block:
    ...
    with Else():
        ...
        with Parallel():
            ...
            Label(text="ORGANIC or INORGANIC", font_size=medium_font, bottom=label1.bottom - label_offset)
            Label(text=f"Press {continue_key} to start.", font_size=small_font, bottom=small_offset)
        with UntilDone():
            KeyPress(keys=continue_key)
        
        StudyPhase(experiment_block.current['study_phase'], session_keys_dictionary)
    ...
    with Parallel():
        ...
        Label(text="Same or Different?", font_size=medium_font, bottom=label1.bottom - label_offset)
        Label(text=f"Press {continue_key} to start.", font_size=small_font, bottom=small_offset)
    with UntilDone():
        KeyPress(keys=continue_key)
    TestPhase(experiment_block.current['test_phase'], session_keys_dictionary)
