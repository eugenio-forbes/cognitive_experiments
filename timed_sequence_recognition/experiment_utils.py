...
def prompt_n_experiment_blocks():
    ...
    while not valid_input:
        valid_input = True
        n_experiment_blocks = input("Enter number of experiment blocks for this session (1-45):")
        if n_experiment_blocks != '':
            if not n_experiment_blocks.isnumeric() or int(n_experiment_blocks) < 1 or int(n_experiment_blocks) > 45:
                valid_input = False
                print("Must enter an integer between 1 and 45. Try again.")
        else:
            valid_input = False
...
def update_list_from_checkpoint(experiment_block_list, checkpoint_file, session_lock_file):
    ...
    with open(checkpoint_file, 'r') as file_handle:
        ...
        for row in csv_dict_reader:
            ...
            last_sequence_index = int(row['sequence_index'])
    ...
    if last_experiment_block is not None:
        if last_experiment_phase == 'ENCODING':
            if last_sequence_index + 1 < len(experiment_block_list[0]['study_phase']):
                study_phase = experiment_block_list[last_experiment_block]['study_phase']
                new_study_phase = study_phase[last_sequence_index + 1:]
